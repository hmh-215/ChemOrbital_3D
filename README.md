...
One-click access to 11 pre-assembled molecular conformations:
...
- **$C_2H_4$ (Ethylene):** Planar $sp^2$ backbone with parallel $p_z$ orbitals connected by a volumetric $\pi$-bond electron cloud.
- **$C_2H_2$ (Acetylene):** Linear $sp$ backbone ($180^\circ$) with two mutually perpendicular in-phase $\pi$ bonds ($\pi_{py}$ and $\pi_{pz}$).
- **Benzene ($C_6H_6$):** Planar $sp^2$ hexagonal ring with continuous delocalized toroidal $\pi$-electron clouds above and below the carbon plane.
...
- **Graphene (3-Layer Honeycomb):** Multilayer hexagonal $sp^2$ lattice with vertical dashed interlayer van der Waals coupling lines.
...
#### C. ⚙️ Selected Atom Properties & Dedicated Per-Axis Rotation
- **Position Sliders (X, Y, Z):** Smooth translation along coordinate axes.
- **Rotation Sliders (X, Y, Z):** Fine-grained rotation from $0^\circ$ to $360^\circ$.
